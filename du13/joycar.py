...
class Robot:
    ...
    def __init__(self, wheelDiameterInMeters: float, axleTrackInMetres: float, i2cBus, neopixel, pin0, pin8, pin12, pin14, pin15):
        self.__wheelDiameterInMeters = wheelDiameterInMeters
        self.__axleTrackInMetres = (axleTrackInMetres / 2)
        self.__i2cBus = i2cBus
        self.motorController = MotorController(self.__wheelDiameterInMeters, self.__axleTrackInMetres, self.__i2cBus)
...
class LineController(Controller):
    ...
    def __init__(self, i2cBus, i2cAddress: bytes=56):
        ...
        self.__clock = ticks_us()
        self.__period = 75000
        self.sensors = {'LEFT': LineSensor('LEFT'), 'CENTER': LineSensor('CENTER'), 'RIGHT': LineSensor('RIGHT')}
    ...
    def checkStatus(self):
        ...
        if ticksDiff > self.__period:
            self.__readData()
            self.__clock = ticksNow
            status = (self.sensors['LEFT'].status, self.sensors['CENTER'].status, self.sensors['RIGHT'].status)
        return status
class Motor:
...
class SpeedController(Controller):
    ...
    def __init__(self, pin14, pin15):
        ...
        self.__pin14 = pin14
        self.__pin15 = pin15
        self.sensors = {'LEFT': SpeedSensor('LEFT', pin14), 'RIGHT': SpeedSensor('RIGHT', pin15)}
...
class MotorController(Controller):
    ...
    def __init__(self, wheelDiameterInMeters: float, axleTrackInMeters: float, i2cBus, i2cAddress: bytes=112):
        super().__init__()
        self.__wheelDiameterInMeters = wheelDiameterInMeters
    ...
    def driveByPWM(self, *args):
        ...
        self.__i2cBus.write(self.__i2cAddress, (self.motors['LEFT'].channelForward + bytes([self.__normalizePWM(self.__pwm0)])))
        self.__i2cBus.write(self.__i2cAddress, (self.motors['LEFT'].channelBackward + bytes([self.__normalizePWM(self.__pwm1)])))
        self.__i2cBus.write(self.__i2cAddress, (self.motors['RIGHT'].channelForward + bytes([self.__normalizePWM(self.__pwm2)])))
        self.__i2cBus.write(self.__i2cAddress, (self.motors['RIGHT'].channelBackward + bytes([self.__normalizePWM(self.__pwm3)])))
    ...
    def driveByVelocity(self, metersPerSecond: float, angleSpeed: float):
        ...
        self.motors['LEFT'].velocity = (metersPerSecond - (self.__axleTrackInMeters * angleSpeed))
        self.motors['LEFT'].velocity /= (self.__wheelDiameterInMeters / 2)
        self.motors['RIGHT'].velocity = (metersPerSecond + (self.__axleTrackInMeters * angleSpeed))
        self.motors['RIGHT'].velocity /= (self.__wheelDiameterInMeters / 2)
        self.motors['LEFT'].pwm = int(((self.motors['LEFT'].paramA * abs(self.motors['LEFT'].velocity)) + self.motors['LEFT'].paramB))
        self.motors['RIGHT'].pwm = int(((self.motors['RIGHT'].paramA * abs(self.motors['RIGHT'].velocity)) + self.motors['RIGHT'].paramB))
    ...
    def driveWithSonar(self, distanceInMetres: float, metersFromObstacle: float=0.2, P: float=0.5):
    ...
    def driveWithLineDetection(self, status: tuple, angleSpeed: float):
        ...
        if (not right) and (not left):
            self.driveByVelocity(self.__metersPerSecondInitial, 0)
